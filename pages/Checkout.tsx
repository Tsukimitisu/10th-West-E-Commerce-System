...
const Checkout: React.FC = () => {
  ...
  const formatPrice = (p: number) => `₱${p.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`;

  const handleApplyPromo = async () => {
    setPromoError('');
    try { await applyDiscount(promoCode); } catch (e: any) { setPromoError(e.message || 'Invalid promo code'); }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/shop" className="text-red-600 hover:text-red-700 text-sm font-medium">Continue Shopping</Link>
        </div>
      </div>
    );
  }
            ...
            <div className="flex-1 space-y-6">
              ...
              <Section title="Shipping Address" icon={<MapPin size={18} />}>
                {addresses.length > 0 && (
                  <div className="space-y-2 mb-4">
                    {addresses.map(addr => (
                      <label key={addr.id} className={`flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition-colors ${selectedAddress === addr.id ? 'border-red-600 bg-red-50' : 'border-gray-200 hover:border-gray-300'}`}>
                        <input type="radio" name="address" checked={selectedAddress === addr.id} onChange={() => { setSelectedAddress(addr.id); setShowNewAddress(false); }} className="mt-1 text-red-600 focus:ring-red-500" />
                        <div>
                          <p className="text-sm font-medium text-gray-900">{addr.recipient_name} {addr.is_default && <span className="text-xs text-red-600 ml-1">Default</span>}</p>
                    ...
                    <button type="button" onClick={() => { setSelectedAddress(null); setShowNewAddress(true); }} className="text-sm text-red-600 hover:text-red-700 font-medium">
              ...
              <Section title="Shipping Method" icon={<Truck size={18} />}>
                <div className="space-y-2">
                  {[
                    { id: 'standard' as const, label: 'Standard Shipping', desc: '3-5 business days', price: subtotal >= 2500 ? 'Free' : '₱150.00' },
                    { id: 'express' as const, label: 'Express Shipping', desc: '1-2 business days', price: '₱300.00' },
                  ].map(method => (
                    <label key={method.id} className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-colors ${shippingMethod === method.id ? 'border-red-600 bg-red-50' : 'border-gray-200 hover:border-gray-300'}`}>
                      <div className="flex items-center gap-3">
                        <input type="radio" name="shipping" checked={shippingMethod === method.id} onChange={() => setShippingMethod(method.id)} className="text-red-600 focus:ring-red-500" />
                        <div>
              ...
              <Section title="Payment Method" icon={<CreditCard size={18} />}>
                <div className="space-y-2 mb-4">
                  {[
                    { id: 'card' as const, label: 'Credit/Debit Card', desc: 'Visa, Mastercard' },
                    { id: 'cod' as const, label: 'Cash on Delivery', desc: 'Pay when you receive' },
                  ].map(method => (
                    <label key={method.id} className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-colors ${paymentMethod === method.id ? 'border-red-600 bg-red-50' : 'border-gray-200 hover:border-gray-300'}`}>
                      <input type="radio" name="payment" checked={paymentMethod === method.id} onChange={() => setPaymentMethod(method.id)} className="text-red-600 focus:ring-red-500" />
                      <div>
                        <p className="text-sm font-medium text-gray-900">{method.label}</p>
                        <p className="text-xs text-gray-500">{method.desc}</p>
                      </div>
            ...
            <div className="lg:w-96">
              <div className="bg-white rounded-xl border border-gray-100 p-6 sticky top-24">
                ...
                <label className="flex items-start gap-2 mt-4 cursor-pointer">
                  <input type="checkbox" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)} className="mt-0.5 text-red-600 focus:ring-red-500 rounded" />
                  <span className="text-xs text-gray-500">I agree to the <a href="#" className="text-red-600 hover:underline">Terms & Conditions</a> and <a href="#" className="text-red-600 hover:underline">Privacy Policy</a></span>
                </label>
                ...
                <button
                  ...
                  className="w-full mt-4 py-3.5 bg-red-600 hover:bg-red-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-all flex items-center justify-center gap-2"
                >
                  {processing ? (
...
const Input: React.FC<{ label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean; placeholder?: string; className?: string }> = ({ label, value, onChange, type = 'text', required, placeholder, className = '' }) => (

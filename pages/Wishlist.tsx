                ...
                <div key={productId} className="bg-white rounded-xl border border-gray-100 overflow-hidden group hover:shadow-md transition-all">
                  ...
                  <div className="relative aspect-square bg-gray-50 overflow-hidden">
                    ...
                    <button onClick={() => handleRemove(productId)}
                      className="absolute top-2 right-2 w-8 h-8 bg-white/90 hover:bg-red-50 rounded-full flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors shadow-sm">
                      <Trash2 size={14} />
                    </button>
                  </div>
                  ...
                  <div className="p-3 space-y-2">
                    <Link to={`/products/${productId}`} className="block">
                      <h3 className="text-sm font-medium text-gray-900 hover:text-red-600 line-clamp-2 transition-colors">{name}</h3>
                    </Link>
                    <div className="flex items-center gap-2">
                      {salePrice ? (
                        <>
                          <span className="font-semibold text-red-600">₱{salePrice.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleAddToCart(item)} disabled={!inStock}
                        className="flex-1 px-3 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-300 text-white text-xs font-medium rounded-lg transition-colors flex items-center justify-center gap-1">

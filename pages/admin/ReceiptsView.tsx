...
const ReceiptsView: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const o = await getOrders(); setOrders(o.filter((o: any) => o.status !== 'cancelled')); } catch (e) { console.error(e); }
      setLoading(false);
    })();
  }, []);
  ...
  const filtered = orders.filter(o => {
    ...
    return !term || o.id.toString().includes(term) || (o as any).customer_name?.toLowerCase().includes(term);
  });
  ...
  const handlePrint = (order: Order) => {
    ...
    w.document.write(`
      <html><head><title>Receipt #${order.id}</title><style>
        ...
        <p>Customer: ${(order as any).customer_name || (order as any).shipping_name || 'Walk-in'}</p>
        <div class="line"></div>
        ${(order as any).items?.map((it: any) => `<p>${it.name || it.product_name} x${it.quantity} — ₱${((it.price || 0) * (it.quantity || 1)).toFixed(2)}</p>`).join('') || '<p>Items unavailable</p>'}
        <div class="line"></div>
        <p class="total">TOTAL: ₱${(order.total_amount || 0).toFixed(2)}</p>
        <p>Payment: ${(order as any).payment_method || 'N/A'}</p>
        <div class="line"></div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-white rounded-lg border border-gray-100 p-1 w-fit">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id as any)} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${tab === t.id ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map(o => (
                    <tr key={o.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">{(o as any).customer_name || (o as any).shipping_name || `User ${o.user_id}`}</td>
                      <td className="px-4 py-3 text-xs text-gray-500">{new Date(o.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3 text-xs text-gray-500 capitalize hidden md:table-cell">{(o as any).payment_method || '—'}</td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
            ...
            {[
              ...
              <div key={field.key}>
                ...
                <input value={(template as any)[field.key]} onChange={e => setTemplate(t => ({...t, [field.key]: e.target.value}))}
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={template.showLogo} onChange={e => setTemplate(t => ({...t, showLogo: e.target.checked}))} className="w-4 h-4 text-red-600 rounded" />
                Show Logo
              </label>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={template.showBarcode} onChange={e => setTemplate(t => ({...t, showBarcode: e.target.checked}))} className="w-4 h-4 text-red-600 rounded" />
                Show Barcode
              </label>
            </div>
            <div>
            ...
            <button className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors w-full">Save Template</button>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="bg-gray-50 rounded-lg p-6 font-mono text-xs max-w-[300px] mx-auto border border-dashed border-gray-300">
              {template.showLogo && <div className="text-center mb-1"><div className="w-8 h-8 bg-red-600 rounded mx-auto mb-1 flex items-center justify-center"><span className="text-white font-bold text-[8px]">10</span></div></div>}
        ...
        {previewOrder && (
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-6 font-mono text-xs max-w-[300px] mx-auto border border-dashed border-gray-300">
              ...
              <p>Customer: {(previewOrder as any).customer_name || 'Walk-in'}</p>
              <div className="border-t border-dashed border-gray-400 my-3" />
              {(previewOrder as any).items?.map((it: any, i: number) => (
            ...
            <div className="flex justify-center gap-2">
              <button onClick={() => { handlePrint(previewOrder); setPreviewOrder(null); }} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-colors flex items-center gap-1"><Printer size={12} /> Print</button>

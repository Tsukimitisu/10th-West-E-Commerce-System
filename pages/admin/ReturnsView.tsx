...
const ReturnsView: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await (getReturns as any)();
  ...
  const handleStatusUpdate = async (ret: ReturnRequest, status: ReturnStatus) => {
    try {
  ...
  const handleRefund = async (e: React.FormEvent) => {
    ...
    try {
      await processRefund(refundTarget.id, refundMethod as 'original' | 'store_credit');
      setRefundModal(false); fetchData();
    } catch (e) { console.error(e); }
  };

  const openRefund = (ret: ReturnRequest) => {
    setRefundTarget(ret);
    setRefundAmount(((ret as any).refund_amount || (ret as any).total || 0).toString());
  ...
  const filtered = returns.filter(r => {
    ...
    const matchSearch = !term || r.id.toString().includes(term) || (r as any).order_id?.toString().includes(term) || r.reason?.toLowerCase().includes(term);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Total Refunded', value: `₱${returns.filter(r => r.status === 'refunded' || r.status === 'completed').reduce((s, r) => s + ((r as any).refund_amount || 0), 0).toLocaleString()}`, icon: <DollarSign size={18} />, color: 'bg-green-50 text-green-600' },
        ].map((kpi, i) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-gray-600">#{(r as any).order_id?.toString().padStart(4, '0') || r.id}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {r.status === 'pending' && (
                        <>
                          <button onClick={() => handleStatusUpdate(r, 'approved' as ReturnStatus)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-green-50 text-gray-400 hover:text-green-600 transition-colors" title="Approve"><CheckCircle2 size={14} /></button>
                          <button onClick={() => handleStatusUpdate(r, 'rejected' as ReturnStatus)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors" title="Reject"><XCircle size={14} /></button>
                        </>
                      )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm font-medium text-gray-900">#{(detailReturn as any).order_id?.toString().padStart(4, '0') || '—'}</p>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg">
            ...
            <div className="flex gap-2 pt-2">
              {detailReturn.status === 'pending' && (
                <>
                  <button onClick={() => { handleStatusUpdate(detailReturn, 'approved' as ReturnStatus); setDetailReturn(null); }} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-medium rounded-lg transition-colors">Approve</button>
                  <button onClick={() => { handleStatusUpdate(detailReturn, 'rejected' as ReturnStatus); setDetailReturn(null); }} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-colors">Reject</button>
                </>
              )}

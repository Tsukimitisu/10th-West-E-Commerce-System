...
const SettingsView: React.FC = () => {
  const [tab, setTab] = useState<'store' | 'tax' | 'shipping' | 'payment' | 'email' | 'receipt'>('store');
  ...
  const Label = ({ children }: { children: React.ReactNode }) => <label className="block text-xs font-medium text-gray-600 mb-1">{children}</label>;
          ...
          <div className="space-y-4 max-w-xl">
            ...
            <div className="space-y-2">
              {[
                ...
                <label key={m.key} className={`flex items-center justify-between p-3 border rounded-lg cursor-pointer transition-all ${(payment as any)[m.key] ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-white'}`}>
                  <span className="text-sm font-medium text-gray-900">{m.label}</span>
                  <input type="checkbox" checked={(payment as any)[m.key]} onChange={e => setPayment(p => ({...p, [m.key]: e.target.checked}))} className="w-4 h-4 text-red-600 rounded" />
                </label>
              ))}
            </div>
            {payment.card && (
          ...
          <div className="space-y-4 max-w-xl">
            ...
            <div className="space-y-2">
              {[
                ...
                <label key={n.key} className={`flex items-center justify-between p-3 border rounded-lg cursor-pointer transition-all ${(email as any)[n.key] ? 'border-green-200 bg-green-50' : 'border-gray-100 bg-white'}`}>
                  <div><p className="text-sm font-medium text-gray-900">{n.label}</p><p className="text-[10px] text-gray-500">{n.desc}</p></div>
                  <input type="checkbox" checked={(email as any)[n.key]} onChange={e => setEmail(em => ({...em, [n.key]: e.target.checked}))} className="w-4 h-4 text-green-600 rounded" />
                </label>
              ))}
            </div>
          </div>

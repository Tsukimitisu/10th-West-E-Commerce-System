...
const InventoryView: React.FC = () => {
  ...
  const [tab, setTab] = useState<'stock' | 'adjustments' | 'alerts'>('stock');
  const [adjustModal, setAdjustModal] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [adjForm, setAdjForm] = useState({ type: 'add' as 'add' | 'remove', quantity: '', reason: 'restock', notes: '' });
  ...
  const openAdjust = (p: Product) => { setSelectedProduct(p); setAdjForm({ type: 'add', quantity: '', reason: 'restock', notes: '' }); setAdjustModal(true); };
  ...
  const reasons = ['restock', 'damaged', 'returned', 'correction', 'shrinkage', 'transfer', 'other'];
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Out of Stock', value: outOfStock.toString(), icon: <Package size={18} />, color: 'bg-red-50 text-red-600' },
        ].map((kpi, i) => (
      ...
      <div className="flex gap-1 bg-white rounded-lg border border-gray-100 p-1 w-fit">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${tab === t.id ? 'bg-red-50 text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>
            <t.icon size={14} />
            {t.label}
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${tab === t.id ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-500'}`}>{t.count}</span>
          </button>
        ))}
      </div>
          ...
          <div className="p-3 border-b border-gray-100">
            <div className="relative max-w-xs">
              ...
              <input type="text" placeholder="Search inventory..." value={search} onChange={e => setSearch(e.target.value)}
                className="w-full pl-9 pr-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20" />
            </div>
          </div>
          {loading ? (
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${status === 'out' ? 'bg-red-500' : status === 'low' ? 'bg-amber-400' : 'bg-green-400'}`} style={{ width: `${pct}%` }} />
                          </div>
                          <span className={`font-bold ${status === 'out' ? 'text-red-600' : status === 'low' ? 'text-amber-600' : 'text-gray-900'}`}>{p.stock_quantity}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-right text-gray-500 hidden sm:table-cell">{p.low_stock_threshold}</td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${status === 'out' ? 'bg-red-50 text-red-600' : status === 'low' ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-green-600'}`}>
                ...
                {adjustments.slice(0, 50).map((a, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${isAdd ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
            ...
            <div className="divide-y divide-gray-50">
              {lowStock.map(p => (
                <div key={p.id} className="flex items-center justify-between p-4 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${p.stock_quantity === 0 ? 'bg-red-50' : 'bg-amber-50'}`}>
                      <AlertTriangle size={16} className={p.stock_quantity === 0 ? 'text-red-500' : 'text-amber-500'} />
                    </div>
                    <div>
                      <p className="font-medium text-gray-900 text-sm">{p.name}</p>
                      <p className="text-xs text-gray-500">Threshold: {p.low_stock_threshold} • Current: <span className={`font-bold ${p.stock_quantity === 0 ? 'text-red-600' : 'text-amber-600'}`}>{p.stock_quantity}</span></p>
                    </div>
                  </div>
                  <button onClick={() => openAdjust(p)} className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-colors flex items-center gap-1">
        ...
        <form onSubmit={handleAdjust} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setAdjForm(f => ({...f, type: 'remove'}))}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium border transition-all ${adjForm.type === 'remove' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
          ...
          <div>
            ...
            <input type="number" min="1" value={adjForm.quantity} onChange={e => setAdjForm(f => ({...f, quantity: e.target.value}))}
              required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20" placeholder="Enter quantity" />
          </div>
          ...
          <div>
            ...
            <textarea value={adjForm.notes} onChange={e => setAdjForm(f => ({...f, notes: e.target.value}))} rows={2}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20" placeholder="Additional details..." />
          </div>

          {adjForm.quantity && (
            <div className={`p-3 rounded-lg text-sm font-medium ${adjForm.type === 'add' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" className={`px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors ${adjForm.type === 'add' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}>

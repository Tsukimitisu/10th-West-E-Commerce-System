...
const statusConfig = {
  ...
  refunded: { icon: AlertTriangle, color: 'text-gray-600', bg: 'bg-gray-900 border-gray-700' },
};
    ...
    <AccountLayout>
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h2 className="font-display font-semibold text-lg text-white flex items-center gap-2"><Package size={20} /> My Orders</h2>
        </div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-orange-500 transition-all"
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-12 text-center">
            ...
            <h3 className="font-semibold text-white mb-1">No orders found</h3>
            <p className="text-sm text-gray-400 mb-4">{search || filter !== 'all' ? 'Try adjusting your filters.' : "You haven't placed any orders yet."}</p>
          ...
          <div className="space-y-3">
            {filtered.map((order) => {
              ...
              return (
                <Link key={order.id} to={`/orders/${order.id}`} className="block bg-gray-800 rounded-xl border border-gray-700 hover:border-gray-700 hover:shadow-sm p-5 transition-all group">
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2 flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        <p className="font-semibold text-white text-sm">Order #{order.order_number || order.id}</p>
                      ...
                      {order.items && order.items.length > 0 && (
                        <div className="flex items-center gap-2 mt-2">
                          {order.items.slice(0, 4).map((item, i) => (
                            <div key={i} className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-700 overflow-hidden flex-shrink-0">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="font-semibold text-white">PHP {Number(order.total || order.total_amount || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</p>

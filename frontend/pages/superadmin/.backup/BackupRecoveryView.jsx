    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-800 flex items-center justify-between">
          ...
          <button onClick={loadBackups} className="text-gray-400 hover:text-white"><RefreshCw size={14} /></button>
        </div>
        {loading ? (
          ...
          <div className="divide-y divide-gray-800 max-h-96 overflow-y-auto">
            {backups.map((backup) => (
              <div key={backup.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-800/30 transition-colors">
                <div className={`p-2 rounded-lg ${backup.status === 'completed' ? 'bg-green-500/10' : backup.status === 'failed' ? 'bg-red-500/10' : 'bg-red-500/100/10'}`}>
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                  ...
                  'bg-red-500/100/10 text-orange-400 border-red-500/30'
                }`}>{backup.status}</span>
              </div>
            ))}
          </div>

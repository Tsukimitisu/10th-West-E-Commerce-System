  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-xl text-white">Receipts</h1>
          <p className="text-sm text-gray-400">Receipt history and template configuration</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-gray-800 rounded-lg border border-gray-700 p-1 w-fit">
        {[
          ...
          <button key={t.id} onClick={() => setTabt.id} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${tab === t.id ? 'bg-red-500/10 text-red-500' : 'text-gray-400 hover:text-gray-700'}`}>
        ...
        <>
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            {loading ? (
              ...
              <div className="p-12 text-center"><FileText size={40} className="mx-auto text-gray-300 mb-3" /><p className="text-sm text-gray-400">No receipts found</p></div>
            ) : (
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50/80 border-b border-gray-700">
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Receipt #</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-400 hidden sm:table-cell">Customer</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-400">Date</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-400 hidden md:table-cell">Payment</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-gray-400">Amount</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-gray-400 w-32">Actions</th>
                </tr></thead>
                <tbody className="divide-y divide-gray-50">
                  {filtered.map(o => (
                    <tr key={o.id} className="hover:bg-gray-50/50">
                      <td className="px-4 py-3"><span className="font-medium text-white font-mono">RCT-{o.id.toString().padStart(4, '0')}</span></td>
      ...
      {tab === 'template' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 space-y-4">
            <h3 className="font-display font-semibold text-white">Template Settings</h3>
            {[
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={template.showLogo} onChange={e => setTemplate(t => ({...t, showLogo: e.target.checked}))} className="w-4 h-4 text-red-500 rounded" />
                Show Logo
              </label>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={template.showBarcode} onChange={e => setTemplate(t => ({...t, showBarcode: e.target.checked}))} className="w-4 h-4 text-red-500 rounded" />
                Show Barcode
              </label>
            </div>
            <div>
            ...
            <button className="px-4 py-2 bg-red-500/100 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition-colors w-full">Save Template</button>
          </div>

          {/* Preview */}
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
            <h3 className="font-display font-semibold text-white mb-4">Preview</h3>
            <div className="bg-gray-900 rounded-lg p-6 font-mono text-xs max-w-[300px] mx-auto border border-dashed border-gray-300">
              {template.showLogo && <div className="text-center mb-1"><div className="w-8 h-8 bg-red-500/100 rounded mx-auto mb-1 flex items-center justify-center"><span className="text-white font-bold text-[8px]">10</span></div></div>}
              <p className="text-center font-bold text-sm">{template.storeName}</p>
              <p className="text-center text-gray-400">{template.tagline}</p>
              <p className="text-center text-gray-400">{template.address}</p>
              <p className="text-center text-gray-400">{template.phone}</p>
        ...
        {previewOrder && (
          <div className="space-y-4">
            ...
            <div className="flex justify-center gap-2">
              <button onClick={() => { handlePrint(previewOrder); setPreviewOrder(null); }} className="px-4 py-2 bg-red-500/100 hover:bg-red-600 text-white text-xs font-medium rounded-lg transition-colors flex items-center gap-1"><Printer size={12} /> Print</button>
              <button onClick={() => setPreviewOrder(null)} className="px-4 py-2 bg-gray-800 border border-gray-700 hover:bg-gray-900 text-gray-700 text-xs font-medium rounded-lg transition-colors">Close</button>
            </div>
          </div>
        )}
      </Modal>

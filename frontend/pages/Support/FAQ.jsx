  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <div className="text-center mb-8">
        <h1 className="font-display font-bold text-2xl text-white mb-2">Frequently Asked Questions</h1>
        <p className="text-gray-400">Find answers to common questions about orders, shipping, returns, and more.</p>
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search questions..."
          className="w-full pl-10 pr-4 py-3 border border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" />
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {categories.map(cat => (
          <button key={cat} onClick={() => { setActiveCategory(cat); setOpenIndex(null); }}
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${activeCategory === cat ? 'bg-red-500/100 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            {cat}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 text-sm">No matching questions found. Try a different search.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map((faq, i) => (
            <div key={i} className={`bg-gray-800 rounded-xl border transition-all ${openIndex === i ? 'border-red-200 shadow-sm' : 'border-gray-700'}`}>
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)}
                className="w-full flex items-center justify-between p-4 text-left">
                <div className="flex items-start gap-3 flex-1">
                  <HelpCircle size={16} className={`mt-0.5 flex-shrink-0 ${openIndex === i ? 'text-red-500' : 'text-gray-400'}`} />
                  <div>
                    <p className={`text-sm font-medium ${openIndex === i ? 'text-white' : 'text-gray-700'}`}>{faq.question}</p>
      ...
      {/* Still need help */}
      <div className="mt-10 bg-gray-900 rounded-xl p-6 text-center">
        <h3 className="font-semibold text-white mb-1">Still need help?</h3>
        <p className="text-sm text-gray-400 mb-4">Our support team is ready to assist you.</p>
        <Link to="/contact" className="inline-flex items-center gap-2 px-5 py-2.5 bg-red-500/100 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition-colors">

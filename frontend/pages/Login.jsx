...
const Login = ({ onLogin }) => {
  ...
  const [oauthProviders, setOauthProviders] = useState({ google: false, facebook: false, loading: true });
  ...
  const handleOAuth = (provider) => {
    if (!oauthProviders[provider]) {
      setError(`${provider === 'google' ? 'Google' : 'Facebook'} login is not configured yet.`);
      return;
    }
    setError('');
    setResendSuccess('');
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.09)] sm:p-8">
          ...
          {!needs2FA ? (
            <>
              {/* OAuth buttons */}
              <div className="space-y-2 mb-6" aria-label="Social login providers">
                ...
                {!oauthProviders.loading && !oauthProviders.google && <p className="text-xs text-slate-500">Google login is not configured yet.</p>}

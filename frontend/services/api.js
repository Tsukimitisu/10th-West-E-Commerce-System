...
const sanitizeClientProductImageUrl = (value, maxLength = 500) => {
  const safeUrl = sanitizeClientHttpUrlOrPath(value, maxLength);
  return safeUrl ? resolveProductImageUrl(safeUrl).slice(0, maxLength) : null;
};

const sanitizeClientProductImageUrlArray = (value, maxItems = 9) => {
  const deduped = new Set();
  const cleaned = [];

  sanitizeClientUrlArray(value, maxItems).forEach((item) => {
    const safeUrl = sanitizeClientProductImageUrl(item, 500);
    if (!safeUrl || deduped.has(safeUrl)) return;
    deduped.add(safeUrl);
    cleaned.push(safeUrl);
  });
...
const mapProductToSupabase = (product) => ({
  ...
  image: sanitizeClientProductImageUrl(product.image, 500),
  ...
  image_urls: sanitizeClientProductImageUrlArray(normalizeProductImageUrls(product.image_urls), 9),
...
const mapOrderFromApi = (order) => {
  ...
  return {
    ...
    payment_provider: order.payment_provider ?? undefined,
    ...
    shipping_zone: order.shipping_zone ?? shippingDetails.shipping_zone ?? undefined,
    shipping_coverage: order.shipping_coverage ?? shippingDetails.coverage ?? 'luzon_only',
    base_shipping_fee: roundCurrency(order.base_shipping_fee ?? shippingDetails.base_shipping_fee ?? 0),
    weight_surcharge: roundCurrency(order.weight_surcharge ?? shippingDetails.weight_surcharge ?? 0),
    distance_surcharge: roundCurrency(order.distance_surcharge ?? shippingDetails.distance_surcharge ?? 0),
    actual_weight_kg: roundCurrency(order.actual_weight_kg ?? shippingDetails.actual_weight_kg ?? 0),
    estimated_distance_km: roundCurrency(order.estimated_distance_km ?? shippingDetails.estimated_distance_km ?? 0),
    distance_class: order.distance_class ?? shippingDetails.distance_class ?? undefined,
    package_class: order.package_class ?? shippingDetails.package_class ?? undefined,
    free_shipping_applied: shippingDetails.free_shipping_applied ?? shippingAmount === 0,

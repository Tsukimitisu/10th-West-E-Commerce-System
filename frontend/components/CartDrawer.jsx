      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl flex flex-col animate-fade-in">
        ...
        {items.length === 0 ? (
          ...
          <>
                ...
                <div key={item.productId} className="flex gap-3 animate-fade-in items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.product.stock_quantity != null && (
                      <p className="text-xs text-gray-600 mt-0.5">Stock: {item.product.stock_quantity}</p>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                        <button onClick={() => handleDecreaseQty(item)} className="px-2 py-1 text-gray-600 hover:bg-gray-100 transition-colors" disabled={item.quantity <= 1}><Minus size={14} /></button>
                        <input 
                          type="text" 
                          inputMode="numeric" 
                          pattern="[0-9]*"
                          ...
                          className="w-10 py-1 text-sm font-medium text-center bg-transparent text-gray-900 focus:outline-none focus:bg-gray-100 transition-colors"
                        />
                        <button onClick={() => handleIncreaseQty(item)} className="px-2 py-1 text-gray-600 hover:bg-gray-100 transition-colors"><Plus size={14} /></button>
                      </div>
            ...
            {/* Footer */}
            <div className="border-t border-gray-300 p-5 space-y-3">
              ...
              <button
                ...
                className="w-full py-3 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-100 transition-colors"
              >
                View Cart
              </button>
              <button
        ...
        {showLoginModal && (
          <div className="absolute inset-0 z-10 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-xs w-full mx-4 p-6">
              <button onClick={() => setShowLoginModal(false)} className="absolute top-3 right-3 p-1 text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-100 transition-colors">
                <X size={18} />
              </button>
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mb-5">You need to sign in to proceed to checkout.</p>
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-100 transition-colors text-sm"
                  >
                    Cancel
                  </button>
                </div>

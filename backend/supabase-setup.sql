...
DO $deprecated$
BEGIN
  RAISE EXCEPTION 'backend/supabase-setup.sql is deprecated; run npm run migrate';
END
$deprecated$;
...
INSERT INTO products (
  ...
  ('2DP-H2129-00',     'Battery Cover',                'Original Yamaha Battery Cover for NMAX V1.',  150.00, 104.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='NMAX V1'),         2,  '2F STAIRS', 2,  'Yamaha', 'SKU-001', '123456789012'),
  ('2DP-F8351-00-P1',  'Body Cowling Pearl White (46)', 'Side body cowling in Pearl White finish.',    800.00, 448.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='NMAX V1'),         4,  '46',        3,  'Yamaha', 'SKU-002', '123456789013'),
  ('2DP-F8351-00-P5',  'Body Cowling Matte Red (37)',   'Side body cowling in Matte Red finish.',      600.00, 560.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='NMAX V1'),         7,  '37',        3,  'Yamaha', 'SKU-003', '123456789014'),
  ('2DP-E4412-00',     'Cap Cleaner Case Outer',        'Outer casing for air cleaner.',               500.00, 343.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='NMAX V1'),         5,  '2F STAIRS', 2,  'Yamaha', 'SKU-004', '123456789015'),
  ('2DP-F2865-00-P7',  'Cover Front Matte Black (43)',  'Front cover panel matte black.',              700.00, 509.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='NMAX V1'),         6,  '43',        3,  'Yamaha', 'SKU-005', '123456789016'),
  ('UNIV-OIL-1040',   'Motul 7100 4T 10W-40',         '100% synthetic 4-stroke lubricant.',           650.00, 450.00, '/images/product-fallback.svg', (SELECT id FROM categories WHERE name='Universal Parts'), 45, 'SHELF A1',  10, 'Motul',  'SKU-006', '123456789017')

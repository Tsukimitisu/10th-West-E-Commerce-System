...
import { StaffMember, Permission, ActivityLog, StaffPerformance } from '../types';
import {
  ...
  UserCheck, UserX, Clock, FileText, BarChart3,
} from 'lucide-react';

type SubView = 'list' | 'detail' | 'add' | 'edit' | 'logs';
...
const StaffManagement: React.FC = () => {
  ...
  const handlePermissionChange = async (permId: number, granted: boolean) => {
    if (!selected) return;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => handleDelete(selected.id)} className="px-3 py-1.5 bg-red-600 text-white rounded-lg text-sm flex items-center gap-1"><Trash2 className="h-3.5 w-3.5" /> Delete</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <div className="flex items-start gap-4">
            <div className="h-16 w-16 rounded-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-white text-xl font-bold">
              {selected.name.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-bold text-slate-900">{selected.name}</h2>
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${selected.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <button onClick={() => handleToggle(selected.id)} className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1 ${selected.is_active ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-green-50 text-green-600 hover:bg-green-100'}`}>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          {Object.entries(grouped).map(([cat, perms]: [string, Permission[]]) => (
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="p-3 rounded-lg bg-red-50 text-red-600 text-sm font-medium">{error}</div>
      )}
              ...
              {staff.map(s => (
                <tr key={s.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-white font-bold text-sm">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => handleToggle(s.id)} className="p-1.5 text-slate-400 hover:text-amber-500" title={s.is_active ? 'Deactivate' : 'Activate'}>
                        {s.is_active ? <ToggleRight className="h-4 w-4 text-green-500" /> : <ToggleLeft className="h-4 w-4 text-red-400" />}
                      </button>
                      <button onClick={() => handleDelete(s.id)} className="p-1.5 text-slate-400 hover:text-red-500" title="Delete"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}

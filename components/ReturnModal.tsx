...
const ReturnModal: React.FC<ReturnModalProps> = ({ isOpen, onClose, order, onSuccess }) => {
  ...
  const updateQty = (productId: number, qty: number, maxQty: number) => {
    if (qty < 1) qty = 1;
  ...
  const refundTotal = Object.entries(selectedItems).reduce((sum, [pid, qty]) => {
    ...
    return sum + (item ? item.product.price * (qty as number) : 0);
  }, 0);
